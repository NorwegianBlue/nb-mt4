//+------------------------------------------------------------------+
//|                                          NB - 60mm Engulfing.mq4 |
//|                                  Copyright © 2012, NorwegianBlue |
//|           http://sites.google.com/site/norwegianbluesmt4junkyard |
/*
  V4
    - Changed default TP_Factor to 1.1
    - Fixed Risk_Pct - risk based lot sizing now works.
    - Added Max_Lots for use when risk based lot sizing is applied.
    - Added MissedByThatMuch_Active_Pips
            MissedByThatMuch_Active_Factor
            MissedByThatMuch_SL_Factor
    - Changed order comment to include the timeframe on which the trade is based
    - Added EnterOnRetrace_Factor. When non-zero, overrides EntryLag_Pips to enter on retrace rather than breakout.

  V3
    - Added EntryLag_Pips.
    - Added IncludeSpreadInLag.
    - Added MoveStopToBE_Factor.
    - Added LockInAtBE_Pips.
    - Fixed Active.
    - Fixed HourStart & HourEnd.  They now work.
    - Renamed EngulfingCanStraddleEMA to EngulfingCanStraddleSMA.
    - Changed default TP_Factor to 1.5.
    - Changed default MoveStopToBE_Factor to 0.4
    - Changed default lot size to 0.5
*/
//+------------------------------------------------------------------+
...
//---- input parameters
extern int _VERSION_4 = _VERSION_;
...
extern double MoveStopToBE_Factor = 0.40;
...
extern double EnterOnRetrace_Factor = 0.70;
extern double EntryLag_Pips = 1.0;
extern bool   IncludeSpreadInLag = true;
extern bool   EngulfingCanStraddleSMA = true;
extern double TP_Factor = 1.1;
...
{
  return("NB - 60mm Engulfing " + Symbol());
}

string GetOrderComment(int magic)
{
  return("NB60E." + PeriodToStr(Period()));
}
