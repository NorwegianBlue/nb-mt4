//+------------------------------------------------------------------+
//|                                  NB - 60mm Commando Adjutant.mq4 |
//|                                 Copyright © 2012, Norwegian Blue |
...
/*
  29/6/2012 FORKED from flag trading EA
  
  v3 - TODO change MoveStopToBEFactor to be a factor of the stop, not a factor of the TP
...
{
  return("NB - 60mm Commando Adjutant " + VERSION_STR + " " + Symbol() + " " + PeriodToStr(Period()));
}
...
int init()
{
  ...
  Magic = CalculateMagicHash();
	...
	string title =  "60mm Commando Adjutant V" + VERSION_STR;
	if (POINT_FACTOR == 10)
	  title = title + " - (5 digit)";
	else
	  title = title + " - (4 digit)";
...
{
  return (iMA(NULL, 0, SMAFilterPeriod, 0, MODE_SMA, PRICE_CLOSE, bar));
}


bool _IsSMAFilterLong()
{
  return (Bid >= _MA());
}


string _GetLongShortButtonText()
{
  if (_IsSMAFilterLong())
    return ("[Long]");
  else
...
void _InitButtons()
{
  ...
  SetLabel("btnLongShort", 1, BTNLONGSHORT_X, BTN_Y, _GetLongShortButtonText(), _BtnColor(ActiveMode == MD_LONGSHORT));
...
void _SLBarTextRefresh()
{
  ...
  if (ObjectFind(pfx+"hbSL")>=0)
  {
    ...
    s = "SL " + DoubleToStr(pips, 0) + " pips";
    SetText("lbSL", Time[0] + (Time[0]-Time[1])*3, price, s, Red);
    ObjectSetText(pfx+"hbSL", s, 0);
  }
  else
    DeleteObject("lbSL");
  ...
  if (ObjectFind(pfx+"hbTP")>=0)
  {
    ...
    s = "TP " + DoubleToStr(pips, 0) + " pips";
    SetText("lbTP", Time[0] + (Time[0]-Time[1])*3, price, s, RoyalBlue);
    ObjectSetText(pfx+"hbTP", s, 0);
  }
  else
    DeleteObject("lbTP");
}
...
void _UICheck()
{
  ...
  else if (_ButtonClicked("btnLongShort", BTNLONGSHORT_X, BTN_Y))
  {
    MultiComment("Activated " + _GetLongShortButtonText(), CommentDelay);
    DeleteObject("btnLongShort");
    _SLBarClear();
    DeleteObject("hbTP");
    DeleteObject("lbTP");
    DeleteObject("hbDeactivate");
    _SetMode(MD_LONGSHORT);
    NeedRedraw = true;
  }
...
void _EnterNow()
{
  return;   // DISABLE ALL TRADING WHILE DEVELOPING
...
// Rules
// If Bar[1] is long
//   AND Bar[3].high > Bar[2].High
//   AND Bar[4].high > Bar[3].High
// THEN
//  Bar[0] is entry candle
//    - Entry is Bar[1].High + Lag
//
// If Bar[1] is short
//   AND Bar[3].low < Bar[2].Low
//   AND Bar[4].low < Bar[3].Low
// THEN
//   Bar[0] is entry candle
//     - 
...
bool _IsSignal(int bar=0)
{
  ...
  if (_BarDirection(bar) > 0)
  {
    if (//Bid > iMA(NULL, 0, 14, 0, MODE_EMA, PRICE_CLOSE, bar) &&
      Bid > _MA(bar)  &&
      High[bar+2] > High[bar+1]  &&  High[bar+3] > High[bar+2]  &&  High[bar+4] > High[bar+3]) // &&  High[bar+5] > High[bar+4])
      direction = +1;
  }
  else if (_BarDirection(bar) < 0)
  {
    if (//Bid < iMA(NULL, 0, 14, 0, MODE_EMA, PRICE_CLOSE, bar) &&
      Bid < _MA(bar) &&
      Low[bar+2] < Low[bar+1]  &&  Low[bar+3] < Low[bar+2]  &&  Low[bar+4] < Low[bar+3]) //  &&  Low[bar+5] < Low[bar+4])
      direction = -1;
  }
  
  if (direction != 0  &&  !CanCrossSMA)
  {
    if ( (_MA(bar) > Low[bar]  &&  _MA(bar) < High[bar])
         || (_MA(bar+1) > Low[bar+1]  &&  _MA(bar+1) < High[bar+1])
         || (_MA(bar+2) > Low[bar+2]  &&  _MA(bar+2) < High[bar+2])
         || (_MA(bar+3) > Low[bar+3]  &&  _MA(bar+3) < High[bar+3])
         || (_MA(bar+4) > Low[bar+4]  &&  _MA(bar+4) < High[bar+4]) )
    {
      direction = 0;
    }
  }
...
bool _IsEntryBar(string& reason, int bar, int& signalBar)
{
  ...
  else
  {
    if (_IsSignal(0))
    {
      reason = "Possibly entry next bar";
      result = false;
    }
    else if (_IsSignal(1))
    {
      signalBar = 1;
      if (_IsSMAFilterLong()  &&  Bid > High[1] + _PriceLag())
        result = true;
      else if (!_IsSMAFilterLong()  &&  Bid < Low[1] - _PriceLag())
        result = true;
      else
      {
        reason = "Entry this candle";
        result = false;
      }
    }
    else
...
bool SaveTicketStats(int ticket)
{
  ...
  while (handle <= 0 && retries > 0)
  {
    ...
    retries--;
    handle = FileOpen("commando-trade-"+ticket+".csv", FILE_CSV|FILE_WRITE, ';');
    if (handle < 0)
...
bool LoadTicketStats(int ticket)
{
  ...
  while (handle <= 0 && retries > 0)
  {
    ...
    retries--;
    handle = FileOpen("commando-trade-"+ticket+".csv", FILE_CSV|FILE_READ, ';');
    if (handle < 0)
      Sleep(250 + (3-retries)*250);
  }
...
{
  FileDelete("commando-trade-"+ticket+".csv");
}
...
void SaveTradeResult(int ticket)
{
  ...
  while (handle <= 0 && retries > 0)
  {
    ...
    retries--;
    handle = FileOpen("commando-trade results.csv", FILE_CSV|FILE_READ|FILE_WRITE, CharToStr(9));
    if (handle < 0)
    {
      err = GetLastError();
      Sleep(250 + (3-retries)*250);
...
string SETTINGS_FILENAME = "commando-trade settings.ini";
