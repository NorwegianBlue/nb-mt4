...
//2. have to have 3 candles from the recent high retrace
...
extern double NominalBalanceForRisk$ = 10000;
...
extern double MoveStopToBE_Factor = 0.30;
extern double LockInAtBE_Pips = 0.0;

extern double Fixed_Lots = 0;
extern double Risk_Pct = 2.0;
extern double Max_Lots = 5.0;

extern double MinStop_Pips = 10.0;

extern double EnterOnRetrace_Factor = 0;
extern double EntryLag_Pips = 1.0;
extern bool   IncludeSpreadInLag = true;
extern bool   CanCrossSMA = false;
...
int init()
{
  ...
  PFLoadPointFactors();
  POINT_FACTOR = PFGetPointFactor(Symbol());
...
int _DoCheckForEntry(string& reason)
{
  ...
  if (_IsSignal(1))
  {
    ...
    if (EnterOnRetrace_Factor > 0)
    {
      ...
      if (_LongAllowed(1))
      {
        retracePrice = (High[1] - Low[1]) * (1.0 - EnterOnRetrace_Factor) + Low[1];
        if (Bid < retracePrice)
        {
          reason = "GO LONG NOW";
          result = OP_BUY;
        ...
        {
          reason = "Waiting for LONG retrace";
          return (-1);
        }
      }
      
      if (result < 0  &&  _ShortAllowed(1))
      {
        retracePrice = (High[1] - Low[1]) * EnterOnRetrace_Factor + Low[1];
        if (Bid > retracePrice)
        {
          reason = "GO SHORT NOW";
          result = OP_SELL;
        ...
        {
          reason = "Waiting for SHORT retrace";
          return (-1);
        }
      }
    }
    else
    {
      ...
      if (_LongAllowed(1))
      {
        ...
        {
          reason = "Waiting for LONG breakout";
          return (-1);
        }
      }
  ...
  {
    reason = "Possible entry NEXT BAR";
    return (-1);
  }
...
double _GetStop(int direction, int bar=0)
{
  double result;
  if (direction == OP_BUY)
  {
    result = Low[bar] - _GetSpread();
  }
  else
  {
    result = High[bar] + _GetSpread();
  }
  return (result);
}
...
int _DoEntry(int direction)
{
  ...
  if (direction == OP_BUY)
  {
    ...
    price = Ask;
    sl = Low[1] - _GetSpread();
    if (MinStop_Pips > 0  &&  PriceToPips(price-sl) < MinStop_Pips)
      sl = price - PipsToPrice(MinStop_Pips);
  ...
  else if (direction == OP_SELL)
  {
    ...
    if (MinStop_Pips > 0  &&  PriceToPips(sl-price) < MinStop_Pips)
      sl = price + PipsToPrice(MinStop_Pips);
